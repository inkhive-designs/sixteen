...
if ( ! function_exists( 'sixteen_content_nav' ) ) :
/**
 * Display navigation to next/previous pages when applicable
 */
function sixteen_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'sixteen' ); ?></h1>
...
endif; // sixteen_content_nav

if ( ! function_exists( 'sixteen_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function sixteen_comment( $comment, $args, $depth ) {
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				...
				<div class="comment-metadata">
					<?php printf(  sprintf( '<cite class="fn">%s</cite> on', get_comment_author_link() )  ); ?>
				...
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'sixteen' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
...
endif; // ends check for sixteen_comment()

if ( ! function_exists( 'sixteen_the_attached_image' ) ) :
/**
 * Prints the attached image with a link to the next attached image.
 */
function sixteen_the_attached_image() {
	$post                = get_post();
	$attachment_size     = apply_filters( 'sixteen_attachment_size', array( 1200, 1200 ) );
...
/**
 * Flush out the transients used in sixteen_categorized_blog
 */
function sixteen_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'sixteen_category_transient_flusher' );
add_action( 'save_post',     'sixteen_category_transient_flusher' );
